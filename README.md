...
06-Nov-2021 Tried a few more things to try to get the radio to accept the session response.  
 ...
 - encryption key using challenge response as sernum input to get key, session key in 256 block - no good

I'm guessing that the radio senses the decryption failure because of mismatching checksums so the session response data must include a checksum and so is most likely to be a 256 byte data block rather than just the session key bytes.
...
Problem is that I don't know what is supposed to be in the session response. I've tried a couple of things previously but since there was a bug with the checksum calculation they need to be tried again.   
...
So far I've tried the following;  
 ...
 - some other form of encryption. Maybe it needs 3DES, or even AES. But how to get the keys since 'sread' doesn't appear to provide them, unless that is what the 'MAC' is supposed to be. The MAC does happen to be three times the length of the other 'keys'. The 'Encryption on Reciva' post claims the encryption of file download check blocks uses 3DES. The python script claims it used AES with the session key. They both appear to agree on the decrypting of the session response needed DES with the challenge response key... so i've run out of ideas again...  
...
23-Oct-2021 So I'd more or less given up trying to get this to work but today I noticed a slightly different message on the radio so I though I'd check the logs and maybe see if the RollingFileAppender thing was finally doing what it said on the can (BTW It appears using 'file' is the cause of the problem, logback attempts to rename the file to the date patterned name and this fails because something has the file open, presumably logback - forking typical). 
I was surprised to see that the constant stream of "HEAD /portal" requests had stopped and been replaced by a stream of "GET /portal-newformat" requests. Inspite of the mapping in the portal war that it should handle "/*" the requests were being given a 404 which explained the new message on the radio, not that it made it work any better - how the hell could the barstewards that wrote that code not expect that the server would be unavailable, and ensure that the radio continued to function normally until there was actually some need for a connection to the server????  
...
## Decrypting the initial POST data
...
The size of the block, ie. the Type 1 block, received with the /session request is puzzling. How come it's 256 bytes if encrypting 256 bytes actually produces 264 bytes of encrypted data? Is it actually encrypted? [Yes, it is, with the key given by sernum when the challenge is input and the content is the challenge response which is also given by sernum.] Seems the encryption algorithm transform affects the size of the encrypted data, if the right one is used then the encrypted data size is the same as the clear data size.
...
The 'Encryption on Reciva' mentions that the data block, ie. sent by the server, DES stream is encrypted independently of the download request block. Maddeningly it doesn't give any details of the encryption used.
